...
use nom::{
    bytes::complete::is_not, bytes::complete::tag, sequence::preceded, sequence::separated_pair,
    IResult,
};
use std::fs::File;
use std::io::SeekFrom;
...
use crate::minecraft::chunk::{Chunk, ChunkCoordinate};
use crate::progress;

pub const CHUNKS_PER_REGION_WH: u32 = 32;
pub const CHUNKS_PER_REGION: u32 = CHUNKS_PER_REGION_WH * CHUNKS_PER_REGION_WH;
...
impl Region {
    fn mca_to_chunks(region_filename: &str) -> std::io::Result<Vec<Chunk>> {
        ...
        for z in 0..CHUNKS_PER_REGION_WH {
            for x in 0..CHUNKS_PER_REGION_WH {
                progress::PROGRESS_BAR.inc(1);

                let mut location_buf = [0u8; 4];
                let mut timestamp_buf = [0u8; 4];

                let location_offset = 4 * (x + (z * 32)) as u64;
    ...
    pub fn new(region_filename: &str) -> std::io::Result<Region> {
        let (x, z) = region_filename_parser(region_filename);
